...
def crop(point, top_down_view, img_size):
    half = int(img_size / 2)
    padding_view = np.pad(top_down_view, ((0, 0), (half, half), (half, half)), mode='constant')
    x, y = point[1] + half, point[0] + half
    xl = x - half; xr = x + half
    yu = y - half; yd = y + half

    return padding_view[:, xl:xr, yu:yd]

def train(agent, stack_frames, img_size, top_down_img_size, level_script, config, exp_path="experiments_rl", seed=-10, eps_steps=1000, max_steps=1000000):
    total_step = 0
    episode = 0
    ...
    while True:
        ...
        top_down = crop(plane_pos, top_down_view, top_down_img_size)
        top_down = preprocess_top_down(top_down, img_size=img_size)
        top_down = top_down.repeat(stack_frames, axis=0)
        state_dict = {"obs":state, "top_down":top_down}
        ...
        while True:
            ...
            state_next_dict = {"obs":state_next, "top_down":top_down}
            ...
            if peek:
                ...
                top_down = crop(plane_pos, top_down_view, top_down_img_size)
                # top_down = preprocess_top_down(top_down, img_size=(192, 192))
                top_down = np.transpose(top_down, (1, 2, 0))

                plt.imshow(top_down)
                # plt.show()

                plt.savefig("top_down_192.png")
                breakpoint()
                peek = False
            ...
            if total_step % 5000 == 0:
                ...
                env = deepmind_lab.Lab(level_script, 
                    ['RGB_INTERLEAVED', 'DEBUG.CAMERA.TOP_DOWN', 'DEBUG.POS.TRANS'], config)
                img_buffer, score = play(env, agent, stack_frames, img_size, top_down_img_size, seed=seed)
...
def play(env, agent, stack_frames, img_size, top_down_img_size, eps_steps=2000, seed=None):
    img_buffer = []
    pos_seq = []
...
if __name__ == "__main__":
    ...
    step = parser.parse_args().test_step
    ...
    if not test:
        # Experiments Path
        exp_path = "experiments_dqn_topdown/"
    ...
    if not test:
        train(agent, stack_frames, img_size, top_down_img_size, level_script, config, model_path, eps_steps=1000, max_steps=250000)
    else:
