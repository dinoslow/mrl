...
if __name__ == "__main__":
    ...
    parser.add_argument('--step', nargs='?', type=int, default=800000 ,help='Chekpoint step.')
    ...
    agent = dqn.DQNAgent(
        ...
        memory_size = 10000,
        batch_size = 32,)
    # agent.load_model(os.path.join(exp_name, "save"), step=500000)

    if not test:
        train(env, agent, stack_frames, img_size, model_path, eps_steps=1000, max_steps=1000000)
    else:
        agent.load_model(os.path.join(exp_name, "save"), step=step)
        eval_path = os.path.join(exp_name, "eval_num4/")
